...
import (
	...
	"github.com/molsbee/clc-cli/server"
...
func Connect(serverAlias string) {
	serverCredentials := server.GetCredentials(serverAlias)
	ipAddress := server.GetDetails(serverAlias).Details.IPAddresses[0].Internal
